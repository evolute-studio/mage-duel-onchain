...
pub impl ByteArraySpanIntoByteArray of core::traits::Into<@ByteArray, ByteArray> {
    #[inline(always)]
    fn into(self: @ByteArray) -> ByteArray {
        format!("{}", self)
    }
}
...
pub impl BoolToString of BoolToStringTrait {
    #[inline(always)]
    fn to_string(self: bool) -> ByteArray {
        if (self) {
            "True"
        } else {
            "False"
...
mod unit {
    use super::{
        ...
        U256IntoByteArray, ContractAddressIntoByteArray, ByteArraySpanIntoByteArray,
    };
