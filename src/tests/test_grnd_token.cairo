...
mod tests {
    ...
    use evolute_duel::{
        ...
        systems::tokens::grnd_token::{
            grnd_token, IGrndTokenDispatcher, IGrndTokenDispatcherTrait,
            IGrndTokenProtectedDispatcher, IGrndTokenProtectedDispatcherTrait,
        },
    };
    ...
    fn deploy_grnd_token() -> (IGrndTokenDispatcher, IGrndTokenProtectedDispatcher) {
        ...
        let grnd_token_protected = IGrndTokenProtectedDispatcher {
            contract_address: grnd_token_address,
        };
    ...
    fn test_reward_player() {
        ...
        assert(
            grnd_token.balance_of(user_address) == REWARD_AMOUNT.into(),
            'Wrong balance after reward',
        );
        assert(
            grnd_token.total_supply() == REWARD_AMOUNT.into(), 'Wrong total supply after reward',
        );
    }
    ...
    fn test_burn_by_burner() {
        ...
        assert(
            grnd_token.balance_of(user_address) == MINT_AMOUNT - burn_amount,
            'Wrong balance after burn',
        );
        assert(
            grnd_token.total_supply() == MINT_AMOUNT - burn_amount, 'Wrong total supply after burn',
        );
    }
    ...
    fn test_approve_works_but_transfer_from_disabled() {
        ...
        assert(
            grnd_token.allowance(owner_address, spender_address) == approve_amount,
            'Wrong allowance',
        );
    }
    ...
    fn test_camel_case_functions() {
        ...
        let user_address = contract_address_const::<USER_ADDRESS>();
        assert(
            grnd_token.balanceOf(user_address) == grnd_token.balance_of(user_address),
            'balanceOf mismatch',
        );
    }
