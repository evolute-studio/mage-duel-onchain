...
mod tests {
    ...
    use evolute_duel::{
        models::{game::{Board}}, types::packing::{GameState},
        libs::{phase_management::{PhaseManagementTrait, PhaseTransitionData}},
        tests::test_helpers::trait_test_helpers::{TraitTestHelpersTrait}, events::{PhaseStarted},
    };

    use dojo::utils::test::{spawn_test_world, deploy_contract};

    fn setup_world() -> WorldStorage {
        let world = spawn_test_world(array![evolute_duel::models::game::board::TEST_CLASS_HASH]);
    ...
    fn test_get_phase_number() {
        assert!(
            PhaseManagementTrait::get_phase_number(GameState::Creating) == 0,
            "Creating should be phase 0",
        );
        assert!(
            PhaseManagementTrait::get_phase_number(GameState::Reveal) == 1,
            "Reveal should be phase 1",
        );
        assert!(
            PhaseManagementTrait::get_phase_number(GameState::Request) == 2,
            "Request should be phase 2",
        );
        assert!(
            PhaseManagementTrait::get_phase_number(GameState::Move) == 3, "Move should be phase 3",
        );
        assert!(
            PhaseManagementTrait::get_phase_number(GameState::Finished) == 255,
            "Finished should be invalid phase",
        );
    }
    ...
    fn test_update_board_phase() {
        ...
        assert!(
            updated_board.phase_started_at >= initial_timestamp,
            "Phase start time should be updated",
        );
        assert!(
            updated_board.last_update_timestamp >= initial_timestamp,
    ...
    fn test_transition_to_creating_phase() {
        ...
        PhaseManagementTrait::transition_to_creating_phase(
            board_id, top_tile, commited_tile, world,
        );
    ...
    fn test_determine_next_phase_after_move_with_committed_tile() {
        ...
        assert!(
            next_phase == GameState::Reveal, "Should transition to Reveal when tile is committed",
        );
    }
    ...
    fn test_phase_transition_updates_timestamps() {
        ...
        PhaseManagementTrait::transition_to_move_phase(
            board_id, Option::Some(1), Option::None, world,
        );
    ...
    fn test_multiple_phase_transitions() {
        ...
        // Start in Creating
        PhaseManagementTrait::transition_to_creating_phase(
            board_id, Option::None, Option::None, world,
        );
        TraitTestHelpersTrait::assert_board_game_state(world, GameState::Creating);

        // Move to Reveal
        PhaseManagementTrait::transition_to_reveal_phase(
            board_id, Option::None, Option::Some(42), world,
        );
        TraitTestHelpersTrait::assert_board_game_state(world, GameState::Reveal);

        // Move to Request
        PhaseManagementTrait::transition_to_request_phase(
            board_id, Option::Some(5), Option::None, world,
        );
        TraitTestHelpersTrait::assert_board_game_state(world, GameState::Request);

        // Move to Move
        PhaseManagementTrait::transition_to_move_phase(
            board_id, Option::Some(5), Option::Some(99), world,
        );
        TraitTestHelpersTrait::assert_board_game_state(world, GameState::Move);
    }
    ...
    fn test_phase_consistency_after_multiple_operations() {
        ...
        PhaseManagementTrait::transition_to_reveal_phase(
            board_id, Option::None, Option::Some(1), world,
        );
        PhaseManagementTrait::transition_after_move(
            board_id, Option::Some(2), Option::Some(3), world,
        );

        // Verify final state
        let final_board: Board = world.read_model(board_id);
        assert!(
            final_board.game_state == GameState::Reveal,
            "Final state should be Reveal due to committed tile",
        );
