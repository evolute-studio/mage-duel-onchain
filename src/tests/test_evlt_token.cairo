...
mod tests {
    ...
    fn test_burn_by_admin() {
        ...
        assert(
            evlt_token.balance_of(user_address) == MINT_AMOUNT - burn_amount,
            'Wrong balance after burn',
        );
        assert(
            evlt_token.total_supply() == MINT_AMOUNT - burn_amount, 'Wrong total supply after burn',
        );
    }
    ...
    fn test_approve_works_but_transfer_from_disabled() {
        ...
        assert(
            evlt_token.allowance(owner_address, spender_address) == approve_amount,
            'Wrong allowance',
        );
    }
    ...
    fn test_camel_case_functions() {
        ...
        let user_address = contract_address_const::<USER_ADDRESS>();
        assert(
            evlt_token.balanceOf(user_address) == evlt_token.balance_of(user_address),
            'balanceOf mismatch',
        );
    }
