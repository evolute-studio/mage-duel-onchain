...
pub impl TileRevealImpl of TileRevealTrait {
    fn validate_tile_reveal_state(
        board: @Board, expected_state: GameState
    ) -> bool {
    ...
    fn perform_complete_tile_reveal_validation(
        ...
        let tile_commitments_entry: TileCommitments = world.read_model((reveal_data.board_id, reveal_data.player));
        let tile_commitments = tile_commitments_entry.tile_commitments;

        if !Self::validate_tile_commitment(tile_commitments, reveal_data.tile_index, reveal_data.nonce, reveal_data.c) {
            return false;
        }

        // Player and turn validations
        let player_data = match PlayerDataTrait::validate_player_and_get_data(board, reveal_data.player, world) {
            Option::Some(data) => data,
            Option::None => {return false;},
        };

        if !TimingTrait::validate_current_player_turn(board, reveal_data.player, player_data.side, world) {
            return false;
        }
