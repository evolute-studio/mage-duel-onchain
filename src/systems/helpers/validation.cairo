...
pub fn is_valid_move(
    ...
    for side in 0..4_u8 {
        ...
        let adjacent_node_position: u32 = (edge_position.try_into().unwrap() + offset)
            .try_into()
            .unwrap();
        ...
        if adjacent_node.node_type != *edges.at(side.into())
            && adjacent_node.node_type != TEdge::None {
            // Edge does not match
            result = false;
            break;
...
mod tests {
    ...
    use super::*;
    use evolute_duel::{
        types::packing::{Tile, TEdge, PlayerSide}, models::scoring::{UnionNode, m_UnionNode},
    };
    ...
    fn place_tile_on_board(
        mut world: WorldStorage, board_id: felt252, col: u8, row: u8, edges: Span<TEdge>,
    ) {
